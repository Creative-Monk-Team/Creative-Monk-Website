<!-- Start Footer -->
<footer class="cs-fooer">
	<div class="cs-fooer_main">
		<div class="container">
			<div class="row">
				<div class="col-lg-3 col-sm-6">
					<div class="cs-footer_item">
						<div class="cs-text_widget">
							<img class="w-40"
								src="<?php echo esc_url(get_template_directory_uri()); ?>/img/icons/icon.png"
								alt="Thumb">
							<p> Your Gateway to Digital Excellence – Where Innovation Meets Success. Elevate Your
								Brand Today with Our Expertise. </p>
						</div>
						<div class="cs-social_btns cs-style1">
							<a href="https://www.linkedin.com/company/creativemonkindia?originalSubdomain=in" class="cs-center">
								<svg width="15" height="15" viewBox="0 0 15 15" fill="none"
									xmlns="http://www.w3.org/2000/svg">
									<path
										d="M4.04799 13.7497H1.45647V5.4043H4.04799V13.7497ZM2.75084 4.2659C1.92215 4.2659 1.25 3.57952 1.25 2.75084C1.25 2.35279 1.40812 1.97105 1.68958 1.68958C1.97105 1.40812 2.35279 1.25 2.75084 1.25C3.14888 1.25 3.53063 1.40812 3.81209 1.68958C4.09355 1.97105 4.25167 2.35279 4.25167 2.75084C4.25167 3.57952 3.57924 4.2659 2.75084 4.2659ZM13.7472 13.7497H11.1613V9.68722C11.1613 8.71903 11.1417 7.4774 9.81389 7.4774C8.46652 7.4774 8.26004 8.5293 8.26004 9.61747V13.7497H5.67132V5.4043H8.15681V6.54269H8.19308C8.53906 5.887 9.38421 5.19503 10.6451 5.19503C13.2679 5.19503 13.75 6.92215 13.75 9.16546V13.7497H13.7472Z"
										fill="white" />
								</svg>
							</a>
							<a href="#" class="cs-center">
								<svg width="13" height="11" viewBox="0 0 13 11" fill="none"
									xmlns="http://www.w3.org/2000/svg">
									<path
										d="M11.4651 2.95396C11.4731 3.065 11.4731 3.17606 11.4731 3.2871C11.4731 6.67383 8.89535 10.5761 4.18402 10.5761C2.73255 10.5761 1.38421 10.1557 0.25 9.42608C0.456226 9.44986 0.654494 9.4578 0.868655 9.4578C2.06629 9.4578 3.16879 9.0533 4.04918 8.36326C2.92291 8.33946 1.97906 7.60183 1.65386 6.5866C1.81251 6.61038 1.97112 6.62624 2.1377 6.62624C2.36771 6.62624 2.59774 6.59451 2.81188 6.53901C1.63802 6.30105 0.757595 5.26996 0.757595 4.02472V3.99301C1.09864 4.18336 1.49524 4.30233 1.91558 4.31818C1.22554 3.85814 0.773464 3.07294 0.773464 2.1846C0.773464 1.70872 0.900344 1.27249 1.12244 0.891774C2.38355 2.44635 4.27919 3.46156 6.40481 3.57262C6.36516 3.38226 6.34136 3.184 6.34136 2.9857C6.34136 1.57388 7.4835 0.423828 8.90323 0.423828C9.64086 0.423828 10.3071 0.733156 10.7751 1.23284C11.354 1.1218 11.9093 0.907643 12.401 0.614185C12.2106 1.20906 11.8061 1.70875 11.2748 2.02598C11.7903 1.97049 12.29 1.82769 12.75 1.62942C12.4011 2.13702 11.9648 2.5891 11.4651 2.95396V2.95396Z"
										fill="white" />
								</svg>
							</a>
							<a href="https://www.youtube.com/@creativemonk8259" class="cs-center">
								<svg width="13" height="9" viewBox="0 0 13 9" fill="none"
									xmlns="http://www.w3.org/2000/svg">
									<path
										d="M12.4888 1.48066C12.345 0.939353 11.9215 0.513038 11.3837 0.368362C10.4089 0.105469 6.5 0.105469 6.5 0.105469C6.5 0.105469 2.59116 0.105469 1.61633 0.368362C1.07853 0.513061 0.65496 0.939353 0.5112 1.48066C0.25 2.4618 0.25 4.50887 0.25 4.50887C0.25 4.50887 0.25 6.55595 0.5112 7.53709C0.65496 8.0784 1.07853 8.48695 1.61633 8.63163C2.59116 8.89452 6.5 8.89452 6.5 8.89452C6.5 8.89452 10.4088 8.89452 11.3837 8.63163C11.9215 8.48695 12.345 8.0784 12.4888 7.53709C12.75 6.55595 12.75 4.50887 12.75 4.50887C12.75 4.50887 12.75 2.4618 12.4888 1.48066V1.48066ZM5.22158 6.36746V2.65029L8.48861 4.50892L5.22158 6.36746V6.36746Z"
										fill="white" />
								</svg>
							</a>
						</div>
					</div>
				</div>
				<div class="col-lg-3 col-sm-6 flex flex-col md:items-center">
					<div class="cs-footer_item">
						<h2 class="cs-widget_title">More Links</h2>
						<ul class="cs-menu_widget cs-mp0">
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/contact')); ?>">Contact Us</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/about')); ?>">About Us</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/services')); ?>">Our Services</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/portfolio')); ?>">Life@CM</a>
							</li>
						</ul>
					</div>
				</div>
				<div class="col-lg-3 col-sm-6">
					<div class="cs-footer_item">
						<h2 class="cs-widget_title">Contact Us</h2>
						<ul class="cs-menu_widget cs-mp0">
							<li>Unit- 11 & 12, 9th Floor, Sushma Infinium, Zirakpur, Gazipur, Punjab 140603</li>
							<li> +91-9463445566</li>
							<li>info@antemeridiembnb.com</li>
						</ul>
					</div>
				</div>
				<div class="col-lg-3 col-sm-6">
					<div class="cs-footer_item">
						<h2 class="cs-widget_title">Our Services</h2>
						<ul class="cs-menu_widget cs-mp0">
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/social-media-agency-in-zirakpur')); ?>">Social Media Agency in Zirakpur</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/seo-agency-in-zirakpur')); ?>">SEO Agency in Zirakpur</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/pay-per-click-agency-in-zirakpur')); ?>">PPC Agency in Zirakpur</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/social-media-agency-in-chandigarh')); ?>">Social Media Agency in Chandigarh</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/seo-agency-in-chandigarh')); ?>">SEO Agency in Chandigarh</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/pay-per-click-agency-in-chandigarh')); ?>">PPC Agency in Chandigarh</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/social-media-agency-in-mohali')); ?>">Social Media Agency in Mohali</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/seo-agency-in-mohali')); ?>">SEO Agency in Mohali</a>
							</li>
							<li>
								<a href="<?php echo esc_url(home_url('/index.php/pay-per-click-agency-in-mohali')); ?>">PPC Agency in Mohali</a>
							</li>
						</ul>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="container">
		<div class="cs-bottom_footer">
			<div class="cs-bottom_footer_left">
				<div class="cs-copyright">Copyright © 2023 Laralink.</div>
			</div>
			<div class="cs-bottom_footer_right">
				<ul class="cs-footer_links cs-mp0">
					<li>
						<a href="<?php echo esc_url(home_url('/index.php/contact')); ?>">Request Quote</a>
					</li>
				</ul>
			</div>
		</div>
	</div>
</footer>

<!-- End Footer -->
<span class="cs-scrollup">
	<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path d="M0 10L1.7625 11.7625L8.75 4.7875V20H11.25V4.7875L18.225 11.775L20 10L10 0L0 10Z" fill="currentColor" />
	</svg>
</span>
<!-- Start Video Popup -->
<div class="cs-video_popup">
	<div class="cs-video_popup_overlay"></div>
	<div class="cs-video_popup_content">
		<div class="cs-video_popup_layer"></div>
		<div class="cs-video_popup_container">
			<div class="cs-video_popup_align">
				<div class="embed-responsive embed-responsive-16by9">
					<iframe class="embed-responsive-item" src="about:blank"></iframe>
				</div>
			</div>
			<div class="cs-video_popup_close"></div>
		</div>
	</div>
</div>

<!-- SIMPLE LIGHTBOX CSS & JS -->
<link href="https://cdn.jsdelivr.net/npm/simplelightbox@2.14.2/dist/simple-lightbox.min.css" rel="stylesheet" />
<script src="https://cdn.jsdelivr.net/npm/simplelightbox@2.14.2/dist/simple-lightbox.min.js"></script>

<script>
  document.addEventListener("DOMContentLoaded", function () {
    const links = document.querySelectorAll('a[rel^="lightbox"]');
    console.log("Simple Lightbox Debug — total lightbox links found:", links.length);

    if (typeof SimpleLightbox === "undefined") {
      console.error("❌ SimpleLightbox JS NOT LOADED!");
    } else {
      console.log("✅ SimpleLightbox JS is available.");
      const lightbox = new SimpleLightbox('a[rel^="lightbox"]', {
        captions: true,
        captionSelector: 'img',
        captionType: 'attr',
        captionsData: 'alt',
        animationSpeed: 250,
      });
      console.log("✅ Lightbox initialized:", lightbox);
    }
  });
</script>


<script src="https://unpkg.com/typed.js@2.1.0/dist/typed.umd.js"></script>
<script>
	var typed = new Typed('#element', {
		loop: true,
		strings: ['Web Designing', 'Digital Marketing', 'Graphic Designing', 'Video Animations'],
		typeSpeed: 50,
		backDelay: 1500,
		showCursor: false,
	});
</script>
<script src="<?php echo esc_url(get_template_directory_uri()); ?>/js/plugins/jquery-3.6.0.min.js"></script>
<script src="<?php echo esc_url(get_template_directory_uri()); ?>/js/plugins/isotope.pkg.min.js"></script>
<script src="<?php echo esc_url(get_template_directory_uri()); ?>/js/plugins/jquery.slick.min.js"></script>
<script src="<?php echo esc_url(get_template_directory_uri()); ?>/js/plugins/jquery.counter.min.js"></script>
<script src="<?php echo esc_url(get_template_directory_uri()); ?>/js/plugins/lightgallery.min.js"></script>
<script src="<?php echo esc_url(get_template_directory_uri()); ?>/js/plugins/wow.min.js"></script>
<script src="<?php echo esc_url(get_template_directory_uri()); ?>/js/plugins/gsap.min.js"></script>
<script src="<?php echo esc_url(get_template_directory_uri()); ?>/js/main.js"></script>
<script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js"
	integrity="sha512-bPs7Ae6pVvhOSiIcyUClR7/q2OAsRiovw4vAkX+zJbw3ShAeeqezq50RIIcIURq7Oa20rW2n2q+fyXBNcU9lrw=="
	crossorigin="anonymous" referrerpolicy="no-referrer"></script>
<script>
	var owl = $('.client-carousel');
	owl.owlCarousel({
		items: 4,
		loop: true,
		margin: 10,
		autoplay: true,
		autoplayTimeout: 2000,
		autoplayHoverPause: true,
		responsive: {
			0: {
				items: 1,
			},
			600: {
				items: 3,
			},
			1000: {
				items: 4,
			}
		}
	});
	$('.play').on('click', function () {
		owl.trigger('play.owl.autoplay', [1000])
	})
	$('.stop').on('click', function () {
		owl.trigger('stop.owl.autoplay')
	})
</script>
<script>
	$(document).ready(function () {
		$('.faq-question').click(function () {
			// Toggle the corresponding answer using slide and fade effects
			$(this).next('.faq-answer').slideToggle(300)
			$(this).css("color", "#ff6900")
			$(this).parent(".faq-item").siblings().children(".faq-answer").slideUp(300)
			$(this).parent(".faq-item").siblings().children().css("color", "")
		});
	});
</script>
<script>
        $(document).ready(function () {
            // Initialize jQuery UI Accordion
            $("#whyus").accordion({
                collapsible: true, // Allows sections to collapse
                active: false, // All sections start closed
                heightStyle: "content", // Adjusts height to fit content
                animate: 200 // Smooth animation
            });

            $(".whyus").click(function () {
                // Reset styles for all headings
                $(".whyus").css("color", "");

                // Change text color for the clicked heading
                $(this).css("color", "#ff6900");
            });
        });

    </script>
</body>

</html>